...
**Вывод для распределённого режима:** агент на каждой базе читает свои буферы локально и
шлёт готовые числа (`serialization.serialize`) главному. Ровно так устроен NIDAS
(main/local, порты `0xADD`/`0x575`). Pull-модель (главный опрашивает по таймеру) + watchdog
`lastSeen` → `OFFLINE`.
...
- **Распределённый режим** (main/node по modem + Linked Card) — обсуждён, не реализован.
  Ждёт ответа: строить ли сейчас и нужна ли межпространственная связь.
